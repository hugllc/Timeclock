<?php
/**
    $Id: install.dfprojectwcomp.php 422 2006-12-31 18:24:37Z prices $
...
function com_install() {
	...
	$prefs = array(
		array(
			...
			'preftype' => 'ADMINUSER',
			'default' => 0,
			'help' => 'Does this person have read access to workers comp codes?',
			'parameters' => array(
				'title' => "Read Access",
			),
		),
		array(
			...
			'preftype' => 'ADMINUSER',
			'default' => 0,
			'help' => 'Does this person have write access to workers comp codes?',
			'parameters' => array(
				'title' => "Write Access",
			),
		),
...
?>	
	<div style="text-align: left; margin-bottom: 50px;">
		...
		<tr>
			<th class="install">
			dfProject Workers Comp Installation
			</th>
		</tr>
		</table>
...
<?php
	...
	if ($found['dfprefs']) {
		...
		foreach ($prefs as $p) {
			...
			$gPref = dfPrefs_define::get($p['name'], NULL, 'com_dfprojectwcomp');			
			$id = $gPref[0]->id;
			$ret = @dfPrefs_define::set($id, $p['name'], $p['default'], $p['type'], $p['preftype'], 'com_dfprojectwcomp', $p['help'], $p['parameters']);
        	if ($ret) {
