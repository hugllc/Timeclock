 ...
 * @link       https://dev.hugllc.com/index.php/Project:ComTimeclock
 */
class ReportModel extends DefaultModel
{    
    ...
    */
    protected function checkTimesheet(&$entry)
    {
        ...
        if (DateHelper::beforeStartDate($entry->worked, $entry->user_id)) {
            $entry->hours = 0;
        }
        if (DateHelper::afterEndDate($entry->worked, $entry->user_id)) {
            $entry->hours = 0;
        }
        // Round the hours
    ...
    */
    ...
    {
        return TimeclockHelper::getHolidayPerc($id, $date);
