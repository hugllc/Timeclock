...
{    
    ...
    private function _getReportID($type = null, $start = null, $end = null)
    {
        if (is_null($this->_report)) {
            ...
            if (is_object($this->_report)) {
                $this->_report->projects = json_decode($this->_report->projects, true);
                $this->_report->users = json_decode($this->_report->users, true);
                $this->_report->timesheets = json_decode($this->_report->timesheets, true);
                $this->_report->customers = json_decode($this->_report->customers, true);
                $this->_report->departments = json_decode($this->_report->departments, true);
            } else {
        ...
        return (int)$this->_report->report_id;
    }
    /**
    ...
    */
    ...
    {
        if (is_null($this->_report)) {
            // This will cause the record to get created.  This is done because
            // we need the type, start and end, which are generated by
            // populateState().
            $this->getState("report_id");
        }
        return $this->_report;
    }
    /**
