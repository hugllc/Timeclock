 ...
 * @link       https://dev.hugllc.com/index.php/Project:ComTimeclock
 */
class PtoModel extends AdminModel
{
    use PtoDBTrait;
    /**
     ...
     * @since  3.2
     */
    public $typeAlias = 'com_timeclock.pto';
    ...
    */
    public function getItem($id = null)
    {
        ...
        $id = $id ? $id : Factory::getApplication()->getInput()->getInt("pto_id");
        if ($id != NULL) {
            $query = $this->_buildQuery();
            $query->where('o.pto_id = ' . (int) $id);
            $db->setQuery($query);
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_timeclock.pto', 'pto', ['control' => 'jform', 'load_data' => $loadData]);
        if (empty($form)) {
            return false;
        }
        return $form;
    ...
    protected function loadFormData()
    {
        ...
        $data = $app->getUserState('com_timeclock.edit.pto.data', []);

        if (empty($data)) {
            $data = $this->getItem();
        }

        $this->preprocessData('com_timeclock.pto', $data);
