            ...
            <thead>
                <tr>
                    ...
                    <th width="1%" style="min-width:55px" class="nowrap center">
                        <?php echo HTMLHelper::_('grid.sort', 'JSTATUS', 'p.published', $listDirn, $listOrder); ?>
                    </th>
                    <th>
                        <?php echo HTMLHelper::_('grid.sort', 'COM_TIMECLOCK_NAME', 'p.name', $listDirn, $listOrder); ?>
                    </th>
                    <th class="center hidden-phone">
                        <?php echo HTMLHelper::_('grid.sort', 'COM_TIMECLOCK_MANAGER', 'manager', $listDirn, $listOrder); ?>
                    </th>
                    <th class="center hidden-phone">
                        <?php echo HTMLHelper::_('grid.sort', 'COM_TIMECLOCK_TYPE', 'p.type', $listDirn, $listOrder); ?>
                    </th>
                    <th class="center hidden-phone">
                        <?php echo HTMLHelper::_('grid.sort', 'JCATEGORY', 'category', $listDirn, $listOrder); ?>
                    </th>
                    <th width="1%" class="nowrap center hidden-phone">
                        <?php echo HTMLHelper::_('grid.sort', 'JGRID_HEADING_ID', 'p.project_id', $listDirn, $listOrder); ?>
                    </th>
                </tr>
            </thead>
            <tbody id="project-list">
                <?php for($i=0, $n = count($this->data);$i<$n;$i++) {
                    if ($this->data[$i]->project_id <= 0) {
                        // Don't display the system categories
                        continue;
                    }
                    echo $this->_projectListView->render(
                        array(
                            "data" => $this->data[$i],
                            "index" => $i,
                        )
