...
?>
                <tr class="row<?php echo $displayData["index"] % 2; ?>" sortable-group-id="<?php echo $displayData["data"]->customer_id?>">
                    <td class="center">
                        <?php echo JHtml::_('grid.id', $displayData["index"], $displayData["data"]->customer_id, $displayData["data"]->checked_out, "id"); ?>
                    </td>
                    <td class="center">
                        <div class="btn-group">
                            <?php echo JHtml::_('jgrid.published', $displayData["data"]->published, $displayData["index"], 'customer.', $canChange, 'cb'); ?>
                        </div>
                    </td>
                    <td class="nowrap has-context">
                        <div class="pull-left hasTooltip" title="<?php print $displayData["data"]->notes; ?>">
                            <?php if ($displayData["data"]->checked_out) : ?>
                                <?php echo JHtml::_('jgrid.checkedout', $displayData["index"], $displayData["data"]->checked_out, $displayData["data"]->checked_out_time, 'customer.', $canCheckin); ?>
                            <?php endif; ?>
                            <?php if (($canEdit || $canEditOwn) && !($displayData["data"]->checked_out)): ?>
                                <a href="<?php echo JRoute::_('index.php?option=com_timeclock&controller=customer&task=edit&id='.(int) $displayData["data"]->customer_id); ?>">
